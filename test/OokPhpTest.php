...
class OokPhpTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $process = $this->runOokPhp('file-doesnt-exist.ook');
    ...
    {
        $process = $this->runOokPhp('file-is-empty.ook');
        $this->assertTrue($process->isSuccessful());
    }

    public function test_it_should_fail_when_the_file_contains_an_odd_number_of_ooks()
    {
        $process = $this->runOokPhp('contains-an-odd-number-of-ooks.ook');
        $this->assertFalse($process->isSuccessful());
    }
    ...
    {
        $process = $this->runOokPhp('file-doesnt-contain-any-ooks.ook');
        $this->assertTrue($process->isSuccessful());
    }

    public function test_it_should_successfully_execute_hello_world_ook()
    {
        $process = $this->runOokPhp('hello-world.ook');
        $this->assertTrue($process->isSuccessful());
    ...
    {
        $process = $this->runOokPhp('contains-an-unbalanced-bang-question.ook');
        $this->assertFalse($process->isSuccessful());
    }
    ...
    {
        $process = $this->runOokPhp('contains-a-question-bang-before-a-bang-question.ook');
        $this->assertFalse($process->isSuccessful());
    }
